...
struct Communicator {
  ...
  template <class TMessage>
    bool SetRequestHandler(const boost::function<void(int, const TMessage&)>& handler) {
      int type_id = TMessage::default_instance().type_id();
      RequestHandlers_t::iterator itFind = request_handlers_.find(type_id);

      if (itFind != request_handlers_.end())
	return false; // A handler already exists
  ...
  /// Return false if it fails.
  template <class TMessage>
    bool ParseMessage(TMessage& msg, boost::asio::streambuf& buf) {
      std::istream is(&buf);
  ...
  template <class TRequest>
	...
	callback(header.request_id(), message);
      }
      else {
